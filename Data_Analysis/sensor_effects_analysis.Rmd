...
Cohen's D effect sizes were also calculated for these pairwise comparisons. The before and after contrast for the control speaker is signficant and has a large effect size.
...
rm(ar_model, ar_mod, artic_rate_plot, emm_artic_rate, overall_ar)
...
Although we saw an interesting trend in VSA, the model above
...
Descriptive statistics for M1 and M2 for /sh/ are presented below. The M1 and M2 values between the two groups appear to be similar and follow a simililar pattern to the results from /s/.
...
A linear mixed effects model for M1 was conducted for /s/ to examine changes across the three time points (before, sensors, after) for the two speaker groups (control and PD). M1 significantly decreased with sensor placement and the PD group had an overall lower M1 values. Pairwise comparisons also showed that the contrast between sensor placement and after placement was signficiant for the control group.
...
None of the fixed effects for the LME model for M1 for /sh/ was significant.
...
The LME model for /sh/ M2 revealed that M2 was significantly higher during sensor placement compared to before sensors. However, M2 after sensor placement did not significantly differ from before sensor placement. Pairwise comparisons also revealed that M2 during sensor placement and after sensor removal was also significantly decreased for control speakers. 
