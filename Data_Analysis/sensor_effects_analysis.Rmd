...
VSA_model <- lmerTest::lmer(log(VSA) ~ time_point * group + sex + age + (1 | speaker_id),
                 data = VSA_modelData)
summary(VSA_model)
anova(VSA_model)
...
```
## /ʃ/
Descriptive results for M1 and M2 for /ʃ/ or "sh" are shown below. The PD speakers appear to have a lower M1 values overall. However, both groups appear to have lower M1 values with sensors compared to before and after sensors. M2 appears to increase slightly with sensors for both PD and control speakers.
...
## /ʃ/
Descriptive results for M2 and M2 for /ʃ/ or "sh" are shown below. The M2 values for /ʃ/ are comparable across the three conditions. Both groups have higher M2 values with sensors, comapred to before and after sensors.
