...
The plot shown below shows the effect size of each pairwise comparison

```{r, message = F, warning = F}

my_pal <- c("#6B0F1A",  "#F57E29", "#71C1B2","#32AEAE", "#138690")
...
ar_effect_size <- eff_size(emm_artic_rate, sigma = sigma(ar_model), edf = df.residual(ar_model))|>
  ...
  aes(y = contrast,
      ...
      shape = sig,
      alpha = effect) +
  geom_linerange(aes(xmin = lower_cl, xmax = upper_cl), size = 2, color = my_pal[4]) +
...
lims_y_ar <- c(0, (base::max(plot_data_ar$conf.high) + base::max(plot_data_ar$conf.high)*.45))
lims_group_ar <- base::max(plot_data_ar$conf.high) + base::max(plot_data_ar$conf.high)*.10
lims_HC_ar <- base::max(plot_data_ar$conf.high) + base::max(plot_data_ar$conf.high)*.05
lims_PD_ar <- base::max(plot_data_ar$conf.high) + base::max(plot_data_ar$conf.high)*.40


artic_rate_plot <- plot_data_ar |>
  ggplot() +
  aes(x = condition,
      y = predicted,
      color = group,
...
M1_s_effect_size <- eff_size(emm_M1_s, sigma = sigma(M1_s_mod), edf = df.residual(M1_s_mod)) |>
  ...
  aes(y = contrast,
      ...
      shape = sig,
      alpha = effect) +
  geom_linerange(aes(xmin = lower_cl, xmax = upper_cl), size = 1.5, color = my_pal[4]) +
  geom_linerange(aes(xmin = ifelse(lower_cl < -.35 & upper_cl > -.35, -.35, NA),
                     xmax = ifelse(upper_cl > .35, .35, upper_cl)),
                 size = 1.5,
                 color = my_pal[3]) +
...
lims_y_M1 <- c(0, base::max(plot_data_M1$conf.high) + base::max(plot_data_M1$conf.high)*.45)
lims_group_M1 <- base::max(plot_data_M1$conf.high) + base::max(plot_data_M1$conf.high)*.10
lims_HC_M1 <- base::max(plot_data_M1$conf.high) + base::max(plot_data_M1$conf.high)*.05
lims_PD_M1 <- base::max(plot_data_M1$conf.high) + base::max(plot_data_M1$conf.high)*.40
...
M1_plot <- plot_data_M1 |>
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                ...
                alpha = .5) +
  geom_signif(data = data.frame(Group = c("Before", "Sensors", "After")),
              aes(y_position = c(rep(lims_HC_M1, 6)),
...
lims_y_M2 <- c(0, base::max(plot_data_M2$conf.high) + base::max(plot_data_M2$conf.high)*.45)
lims_group_M2 <- base::max(plot_data_M2$conf.high) + base::max(plot_data_M2$conf.high)*.10
lims_HC_M2 <- base::max(plot_data_M2$conf.high) + base::max(plot_data_M2$conf.high)*.05
lims_PD_M2 <- base::max(plot_data_M2$conf.high) + base::max(plot_data_M2$conf.high)*.40
...
M2_plot <- plot_data_M2 |>
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                ...
                alpha = .5) +
  geom_signif(data = data.frame(Group = c("Before", "Sensors", "After")),
              aes(y_position = c(rep(lims_HC_M2, 6)),
                  xmin = c(1.1, 2.1, 1.1, 1.1, 2.1, 1.1),
                  xmax = c(1.9, 2.9, 2.9, 1.9, 2.9, 1.1),
...
# Exporting Artic Rate Plot

ggsave("Figures/artic_rate_plot.png", plot = artic_rate_plot)

# Exporting VSA Plot

ggsave("Figures/VSA_plot.png", plot = VSA_plot)
...
# Clearning the enviornment from all acoustic data
rm(list=ls())
