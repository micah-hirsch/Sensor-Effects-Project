...
VSA_measure <- VSA_coords %>%
  dplyr::mutate(VSA = geometry::polyarea(mean_F1, mean_F2)) %>%
  dplyr::mutate(timePoint = as.factor(timePoint),
                timePoint = fct_relevel(timePoint, "before", "sensors", "after")) %>%
  dplyr::left_join(participants, by = "SpeakerID") %>%
...
plot_VSA <- VSA_plot %>%
  ...
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin = VSA_mean - se, ymax = VSA_mean + se)) +
  labs(x = "Time Point", y = bquote('Vowel Space Area'(Hz^2))) +
  theme_classic()
...
msc_plot <- ggarrange(M1_plot, plot_VSA, ncol = 1, nrow = 2)

msc_plot

ggsave("msc_plot.png", plot = last_plot(), width = 7, height = 10, units = "in", scale = 0.7)
